...
module.exports = {
    name: 'remove-level',
    description: 'Remove Level command',
    ...
    async execute(client, message, args) {
        ...
        if(!message.member.hasPermission("MANAGE_GUILD")) return message.reply(`No tienes permiso para usar este comando!`);
        ...
        if(!user) {
            ...
            if(isNaN(levelArgs) || levelArgs < 1) {
                ...
                if(score.level - levelArgs < 1) {
                    return message.reply(`Tu no puedes remover los niveles de este usuario!`)
                }    
 		score.level -= levelArgs
                const newTotalXP = levelArgs - 1
                let embed = new Discord.MessageEmbed()
                .setTitle(`Listo!`)
                .setDescription(`Se removieron correctamente ${levelArgs} niveles de ${user.toString()}!`)
                .setColor("RANDOM");
