...
module.exports = {
    name: 'emoji',
    description: 'Emojis command',
    usage: '',
    cooldown: 5,
    guildOnly: true,
    async execute(client, message, args) {
        if (message.guild.emojis.cache.size === 0)
                return message.channel.send(':x: Este servidor no tiene emojis!');
        ...
        if (!args.length) {
                        ...
                        case '⏪':
                            if (page != 1) {
                                ...
                                const embed = new MessageEmbed()
                                    .setTitle(':grinning: Emojis del servidor')
                                    .setColor('RANDOM')
            ...
            async function getEmojiById() {
                ...
                if (emoji) {
                    const embed = new MessageEmbed()
                        .setTitle('Emoji Info')
                        .setColor('RANDOM')
                        .addField('Animado:', `\`${emoji.animated ? 'Si' : 'No'}\``, true)
                        .addField('Creado el:', `\`${moment(emoji.createdAt).format('L')}\``, true)
            ...
            if (!emojiArr.length)
                return message.channel.send(':x: Emoji no encontrado!');

            const embed = new MessageEmbed()
                .setTitle(':grinning: Emojis del servidor')
                .setColor('RANDOM')
                .setDescription('Haga clic de en un emoji para tener información.\n\n' + getEmojiMessage2())
                .setTimestamp()
                .setFooter(message.author.tag, message.author.displayAvatarURL({ dynamic: true }))
