...
module.exports = {
  ...
  async execute(client, message, args) {
    ...
    if(!message.guild.members.cache.get(user.id).roles.cache.has(role.id)) return message.reply(":x: El usuario no tiene ese rango.")
    message.guild.members.cache.get(user.id).roles.remove(role.id).catch(e => message.reply(e))
    message.reply(`:white_check_mark: Al usuario ${user} se le removió el rol ${role} correctamente.`)
